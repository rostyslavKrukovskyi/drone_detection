...
# Load YOLOv5 model
model = torch.hub.load('ultralytics/yolov5', 'custom', path='/Users/rostyslav.krukovskyi/code/openCV/Drone_detection/model.pt', source='github', force_reload=True
)
print(' MODEL NAMES', model.names)

# Set video source (video file path)
...
while True:
    ...
    ret, frame = cap.read()
    if not ret:
        print("End of video file or unable to read video")
        break
    ...
    for result in results.xyxy[0]:
        ...
        if conf > 0.5 and class_name in classes:  # Check confidence and label
            ...
            if rectangle_coords[0] != rectangle_coords[1]:
                if any(
                    rectangle_coords[0][0] <= x <= rectangle_coords[2][0]
                    and rectangle_coords[0][1] <= y <= rectangle_coords[2][1]
